...
while True:
    ...
    # Don't exit on 'result' — with --include-partial-messages, it can
    # arrive before Claude finishes background agents. Wait for EOF
    # (FIFO close = Claude actually exited) as the reliable signal.
    if msg_type == "result":
        end_block()
        continue
