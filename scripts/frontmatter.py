...
Usage:
    ...
    python3 scripts/frontmatter.py set artifacts/epic-tasks/RHAISTRAT-1665-E001.md \\
        epic_id=RHAISTRAT-1665-E001 title="Add text search" \\
        strategy_key=RHAISTRAT-1665 target_repo=opendatahub-io/odh-dashboard \\
        status=Pending

    # Set nested fields with dot notation
    python3 scripts/frontmatter.py set artifacts/codegen-reviews/RHAISTRAT-1665-E001-review.md \\
        scores.lint=2 scores.tests=2
...
def _detect_schema_type(path):
    ...
    if "/codegen-reviews/" in path or "codegen-reviews/" in path:
        return "codegen-review"
    if "/codegen-runs/" in path or "codegen-runs/" in path:
        return "codegen-run"
    if "/epic-tasks/" in path or "epic-tasks/" in path:
        return "epic-task"
    return None
...
def cmd_rebuild_index(args):
    ...
    print(f"Rebuilt {args.artifacts_dir}/epics.md")
...
def main():
    ...
    p_read.add_argument("--schema-type", dest="schema_type",
                        choices=list(SCHEMAS.keys()),
                        help="Schema type (auto-detected from path)")
    ...
    p_set.add_argument("fields", nargs="+",
                       help="Fields as field=value pairs. Use dot notation "
                            "for nested fields (e.g., scores.lint=2)")
